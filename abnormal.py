...
# Define normal ROM ranges
normal_rom = {
    'flexion': (117, 180),
    'extension': (28, 80),
    'abduction': (117, 189),
    'internal_rotation': (30, 110),
    'external_rotation': (40, 117)
}
...
def is_rom_normal(movement, value):
    if movement in normal_rom:
        return normal_rom[movement][0] <= value <= normal_rom[movement][1]
    return True  # For movements not defined in normal_rom
    ...
    if results.pose_landmarks:
        ...
        if calibrated:
            ...
            for movement, value in max_values.items():
                color = (0, 255, 0) if is_rom_normal(movement, value) else (0, 0, 255)
                status = "Normal" if is_rom_normal(movement, value) else "Abnormal"
                draw_text_with_background(frame, f"Max {movement.capitalize()}: {value:.2f} - {status}", (10, y_offset), 0.7, 2, color, (0, 0, 0))
                y_offset += 30
